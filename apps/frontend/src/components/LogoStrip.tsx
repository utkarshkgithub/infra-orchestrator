...
export default function LogoStrip() {
  ...
  return (
    <div ref={sectionRef} className="border-y border-[rgba(232,230,225,0.05)] py-12 overflow-hidden">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="flex w-max gap-4 animate-logo-scroll">
          {items.map((s, i) => (
            ...
            >
              <span className="text-base leading-none text-phosphor">{s.icon}</span>
              <span className="font-sans text-sm font-medium text-ash">{s.name}</span>
            </div>
          ))}
        </div>
      </div>
